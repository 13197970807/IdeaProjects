...
public class Josephu {
    public static void main(String[] args) {
        ...
        //测试小孩出圈是否正确
        circleSingleLinkedList.countBoy(1, 2, 5); //运行结果: 2 -> 4 -> 1 -> 5 -> 3
...
class CircleSingleLinkedList {
    ...
    public void addBoy(int nums) {
        //nums表示要添加几个小孩
        //接下来我们对nums的数据校验,不能传一个负数。
        if(nums < 1) {
            System.out.println("nums的不正确");
            return;
        }
        Boy curBoy = null; //这是一个辅助指针，帮助我们构建环形链表
    ...
    * */
    public void countBoy(int startNo, int countNum, int nums) {
        ...
        //当小孩报数时，让first 和 helper 指针同时的移动 n-1次
        while(true) {
